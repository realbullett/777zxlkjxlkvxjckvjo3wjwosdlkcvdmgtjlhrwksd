...
export default async function handler(req, res) {
  ...
  try {
    const Ins = await Db.execute({
      sql: "INSERT INTO users (provider, provider_id, username, email, password_hash, email_verified, accent_color, text_color, background_color, icon_color, bg_effect_color, primary_color, secondary_color, panel_opacity, signup_ip) VALUES ('email', ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, 0, ?)",
      args: [String(Pending.email), String(Pending.username), String(Pending.email), String(Pending.password_hash), "rgba(255, 255, 255, 0.05)", "#ffffff", "#080808", "#ffffff", "rgba(255, 255, 255, 0.08)", "rgba(255, 255, 255, 0.1)", "rgba(255, 255, 255, 0.15)", String(Pending.ip || "unknown")]
    });
  ...
  await NotifySignup({ Uid, Username: String(Pending.username), Provider: "email" });
