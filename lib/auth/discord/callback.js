...
export default async function handler(req, res) {
  ...
  try {
        ...
        if (Legacy) {
          ...
          try {
            const Ins = await Db.execute({
              sql: "INSERT INTO users (provider, provider_id, username, email, avatar_url, discord_id, accent_color, text_color, background_color, icon_color, bg_effect_color, primary_color, secondary_color, panel_opacity, signup_ip) VALUES ('discord', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)",
              args: [DiscordId, Username, User.email || null, AvatarUrl, DiscordId, DefaultColors.accent_color, DefaultColors.text_color, DefaultColors.background_color, DefaultColors.icon_color, DefaultColors.bg_effect_color, DefaultColors.primary_color, DefaultColors.secondary_color, ReqIp]
            });
    ...
    if (CreatedName) await NotifySignup({ Uid, Username: CreatedName, Provider: "discord" });
