...
async function UpsertUser(Provider, ProviderId, Username, Email, AvatarUrl, Db, Ip) {
  ...
  if (Existing) {
    ...
    return { Uid: Number(Existing.id), Created: false, Username: String(Existing.username || "") };
  }
  ...
  const Ins = await Db.execute({
    sql: "INSERT INTO users (provider, provider_id, username, email, avatar_url, accent_color, text_color, background_color, icon_color, bg_effect_color, primary_color, secondary_color, panel_opacity, signup_ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)",
    args: [Provider, String(ProviderId), FinalName, Email || null, AvatarUrl || null, "rgba(255, 255, 255, 0.05)", "#ffffff", "#080808", "#ffffff", "rgba(255, 255, 255, 0.08)", "rgba(255, 255, 255, 0.1)", "rgba(255, 255, 255, 0.15)", Ip || "unknown"]
  });
  return { Uid: Number(Ins.lastInsertRowid), Created: true, Username: FinalName };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      const R = await UpsertUser("google", String(User.id), User.name, User.email, User.picture, Db, ReqIp);
      Uid = R.Uid;
      if (R.Created) CreatedName = R.Username;
    } catch (UpErr) {
      console.error("Google upsert error:", UpErr);
    }
    if (!Uid) { res.status(500).json({ error: "User creation failed" }); return; }
    const SuspRs = await Db.execute({ sql: "SELECT suspended FROM users WHERE id = ? LIMIT 1", args: [Uid] });
    if (Number(SuspRs.rows?.[0]?.suspended || 0) === 1) {
      res.writeHead(302, { Location: `${AppUrl}/auth?suspended=1` });
      res.end();
      return;
    }
    const Signed = SignUid(Uid);
    res.setHeader("Set-Cookie", `sl_session=${Signed}; HttpOnly; SameSite=Lax; Max-Age=${30 * 24 * 60 * 60}; Path=/`);
    if (CreatedName) await NotifySignup({ Uid, Username: CreatedName, Provider: "google" });
